...
export default async function SourcePage({ params, searchParams }: { params: Promise<{ versionId: string }>; searchParams: Promise<{ at?: string }> }) {
  if (!evidenceEnabled()) notFound();
  const { versionId } = await params; const { at } = await searchParams; const firm = await getCurrentFirm();
