...
- [x] T17 Docs: `docs/real-data.md` ledger/Neraca/Kurs section + SMBC note, `docs/demo/chickin-demo.md`, README feature table — accept: runbook steps match the UI
...
- T17: `docs/real-data.md` (entity currency + PRK in Tambah klien, combined PDFs, §5 ledger/Neraca import, §6 Kurs + Gabungan in Rupiah + revaluation, §7 `verify:real`), `docs/demo/chickin-demo.md` (private walk; screens, no figures), README feature table + commands, CLAUDE.md repo map + skill routing, accounting-rules skill description.
...
- End of cycle: `npm run lint` + `npm run typecheck` clean; `npm test` → Test Files 23 passed, Tests 117 passed; `npm run build` → Compiled successfully; `npm run verify:books` → `ALL PASS — 1333 pemeriksaan saldo cocok dengan ground truth.` (on a freshly reset demo: running it right after the e2e walk fails 1999 for Budi Santoso because the investor walk reviews demo transactions — expected, same as T2); `npm run test:e2e` → 3 passed.

## Ship Notes
- **Migrations (additive):** `20260924094628_ledger_import_multicurrency` (SourceAccount, LedgerImport, ImportCheck, ExchangeRate, entry/line source + fx fields, `JournalLine_fx_check`, enum values, backfills 3900/7190/7200) and `…_account_mapping` (SourceAccount suggestion fields, AiAccountMap, backfills 11 generic template accounts). Applied by `vercel-build` (`prisma migrate deploy`) on each Neon branch. Existing IDR data is unchanged (`functionalCurrency` defaults to IDR).
- **Env vars:** none new. AI mapping uses the existing key/caps.
- **Manual steps:** after merge, sync the private preview: `git push origin main:real-data`. Before the Chickin demo, confirm the SGD→IDR rates in `data/private/rates.csv` (ECB reference rates via api.frankfurter.dev; swap in BI rates if preferred) and load with `npm run verify:real -- chickin --into-app` locally, or import the workbook through the UI on `real-data`.
- **Not verified here:** the Chickin workbook's own TB/FS tabs have no saved values (formulas never recalculated), so Buku was checked against an independent recompute of its ledger plus the consultant's HoldCo 2023 total (S$ 7.136.663,22), not against saved TB/FS figures. Opening the workbook in Excel and saving would let `verify:real` be extended to the TB tab.
- **Dev note:** the desktop app's preview launcher couldn't start `npm run dev` in this folder (`EPERM uv_cwd`, macOS folder permission); the dev server was started from the shell instead.
- **Rollback:** revert the PR. The migrations are additive; the new tables/columns can stay (unused) or be dropped in a follow-up migration.
