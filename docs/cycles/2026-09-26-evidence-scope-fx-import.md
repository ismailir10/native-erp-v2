...
- Review fixes (Codex on #20): `sourceScope` resolves a confirmed entity-column selection's labels (`selectionEntityIds`, now exported) and excludes the unit for an entity the column doesn't name; `stageImport` compares every distinct row rate with Kurs (one per date still persisted) and the REVIEW counts distinct dates.
...
- End of cycle: lint ✓ · typecheck ✓ · `Test Files 48 passed (48) · Tests 371 passed (371)` · `npm run build` ✓ · `verify:books` **ALL PASS — 1333 pemeriksaan saldo cocok dengan ground truth** · `test:e2e` **9 passed (30.4s)**.
- Review fixes: two new DB tests fail without the fix, pass with it. Gate: lint ✓, typecheck ✓, `Test Files 48 passed (48) · Tests 373 passed (373)`.

## Ship Notes
- **No migrations, env vars or dependencies. No AI calls in tests.** `table.currencies` is new optional JSON in `EvidenceVersion.units`; older extractions show *Baris valas* for every ledger sheet until re-read.
- Accounting rules 6a/6b extended (approved in Spec): conversion residue of a per-currency-balanced journal → 7190; file rates only fill empty Kurs dates.
- Behaviour change: evidence questions now search unconfirmed sheets when a period is set (answers say how many); a discarded ledger draft can be prepared again from Dokumen.
- Rollback: revert the merge; no data migration to undo. Rates written by earlier imports are not rewritten.
- Model: local runs use `kimi-k3` (config only). Production Pengaturan unchanged.
