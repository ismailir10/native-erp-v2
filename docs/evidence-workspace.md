...
- Dashboard scope filters collections by client; company selection requires confirmed current evidence selections. Collections can span periods, stated in the UI. Questions and reports retain the chosen period. The question *Periode* is its own choice (URL `tanya`), empty by default — the app-wide period never narrows questions. A period (chosen, or a date in the question) leaves out only sheets known to be outside it — confirmed for another entity or with a known period outside the range; sheets not yet confirmed are still searched and the answer says how many.
