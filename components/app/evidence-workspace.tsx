...
export function EvidenceWorkspace({ initial, clients }: { initial: Workspace; clients: Clients }) {
  ...
  return <div className="space-y-6">
    ...
    <Card><CardHeader><CardTitle>File dan peran sumber</CardTitle></CardHeader><CardContent className="space-y-4">
      <div className="flex flex-wrap gap-2"><Input aria-label="Cari file" placeholder="Cari nama file atau folder…" value={fileSearch} onChange={e => { setFileSearch(e.target.value); setFilePage(0); }} className="min-w-0 flex-1" /><select aria-label="Tampilkan file" className={selectClass} value={fileFilter} onChange={e => { setFileFilter(e.target.value); setFilePage(0); }}><option value="included">File disertakan</option><option value="errors">Perlu tindakan</option><option value="excluded">Tidak disertakan</option><option value="all">Semua item dan folder</option></select></div>
